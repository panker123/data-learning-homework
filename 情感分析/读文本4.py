...
def dic_adv():#构建程度词词典
    file = pd.read_excel(r'C:\Users\hp\Desktop\情感副词词典.xlsx', encoding='ft')
    data2 = []
    file.fillna("", inplace=True)
    for i in file.index.values:
        df_line = file.loc[i, ['情感分类', '情感强度', '情感极性', '情感得分']].to_dict()
...
def similarity(sentence):
    topic1=pd.read_excel(r'C:\Users\hp\Desktop\主题分类关键词.xlsx',header=None)
    pynlpir.open()
...
def items_create():
    ...
    text = pd.read_excel(r'C:\Users\hp\Desktop\峨眉山精选1.xlsx', encoding='ft',header=None)
    topic = pd.read_excel(r'C:\Users\hp\Desktop\主题分类关键词.xlsx', encoding='ft', header=None)
...
#    print(topic[1][3])
    ...
    f1=pd.DataFrame(np.ones((text.shape[0],text.shape[1])))
#    print(flag)
...
#            flag[k][l] = 1
    ...
    for kk in range(i_length):#kk:文本列数53
        for k in range(i_i_length):#k:文本行数913
            text_sentence=text[kk][k]
            for l in range(j_length):#l:主题数10
...
def score_add(sentence_list,dic,dic_adv):

    dic_1=['不是','不建议','不能','否','没']#否定词
    feeling = 0 #中性
    flag_set = 1
    result_list = []
...
#                if key in ['inf', 'infinity', 'INF', 'INFINITY', 'True', 'NAN', 'nan', 'False', '-inf', '-INF', '-INFINITY', '-infinity', 'NaN', 'Nan']:
                ...
                score += s
        for i in dic_1:#进行否定词计算
             if i in word:
                 score=score*-1
        '''for i in dic_adv.keys():#程度副词得分
            if i in word:
                score=score*float(dic_adv[i]['情感得分'])
                print(score)'''
        if score>0:
            feeling = 1
        elif score<0:
...
if __name__ == '__main__':
    ...
    dictionary_adv=dic_adv()
    ggg = []
    aaa = []
    for item in items:
        aaa = score_add(item,dictionary,dictionary_adv)
        ggg.append(aaa)
    df = pd.DataFrame(ggg)
    df.to_excel(r'C:\Users\hp\Desktop\峨眉山主题情感得分2.xlsx', index=False)
